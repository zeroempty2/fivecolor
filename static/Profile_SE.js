...
function save_comment() {
    ...
    let name_SE = $('#name').val()
    let time_SE = new Date().toLocaleString('ko-KR')
    let password_SE = $('#pwd').val()
    $.ajax({
        type: 'POST',
        url: '/comments_SE',
        data: {comment_give: comment_SE, name_give: name_SE, time_give: time_SE, password_give: password_SE},
        success: function (response) {
            alert(response['msg'])
            window.location.reload();
        }
